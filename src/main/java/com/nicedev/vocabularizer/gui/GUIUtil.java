...
public class GUIUtil {
	...
	//stackoverflow-impl
	public static PopupWindow updateContextMenu(String context, BiConsumer<ContextMenuContent, Optional<String>> menuItemsAppender){
				...
				if(window.getScene() != null && window.getScene().getRoot() != null) {
					...
					if(root.getChildrenUnmodifiable().size() > 0) {
						...
						if(popup.lookup(".context-menu") != null) {
							...
							menuItemsAppender.accept(cmc, Optional.of(context));
	...
	public static void addMenuItem(MenuItem menuItem, Object menu) {
		switch (menu.getClass().getSimpleName()) {
			case "Menu": ((Menu) menu).getItems().add(menuItem);
				break;
			case "ContextMenu": ((ContextMenu) menu).getItems().add(menuItem);
				break;

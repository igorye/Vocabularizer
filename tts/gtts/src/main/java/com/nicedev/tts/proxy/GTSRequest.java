...
public enum GTSRequest {
	...
	TEXT_TO_SPEECH ("https://%s/translate_tts?ie=UTF-8&q=%s&tl=en-%s&total=%d&idx=2&textlen=%4$d&client=tw-ob&ttsspeed=0.7"),
	...
	TRANSLATE_DETAILED("https://%s/translate_a/single?client=t&sl=%s&tl=%s&hl=%2$s&dt=at&dt=bd&dt=ex&dt=ld&dt=md&dt=qca" +
			          "&dt=rw&dt=rm&dt=ss&dt=t&ie=UTF-8&oe=UTF-8&source=btn&srcrom=0&ssel=0&tsel=0&kc=0&q=%s&tk=%s");
	...
	public String encode( String host, String textToSpeak, String accent, int length ) {
		return String.format(format,
							 host,
							 URLEncoder.encode(textToSpeak, StandardCharsets.UTF_8),
							 accent,
							 length,
							 length
		);

...
import com.nicedev.tts.proxy.TTSRequestProxy;
...
public abstract class TTSService extends Thread {
	...
	volatile      AtomicInteger           executionLimit = new AtomicInteger(0);
	...
	public void release(int cyclesBeforeRelease) {
		awaitInput = false;
		executionLimit.set(Math.min(cyclesBeforeRelease, outputQueue.size() + inputQueue.size()));
		isLimited = true;
